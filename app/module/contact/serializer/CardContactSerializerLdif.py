...
from app.module.contact.serializer.CardContactSerializer import CardContactSerializer
...
class CardContactSerializerLdif(CardContactSerializer[str]):
    ...
    def serialize(self, data: CardContact) -> str:
        ...
        lines.append(FormatEngineLdif.emit_attr(ld.ATTR_SN, data.last_name or common_name))  # sn is mandatory
        if data.first_name:
            lines.append(FormatEngineLdif.emit_attr(ld.ATTR_GIVENNAME, data.first_name))
        if data.display_name:
            lines.append(FormatEngineLdif.emit_attr(ld.ATTR_DISPLAYNAME, data.display_name))
        for email in data.emails:
            lines.append(FormatEngineLdif.emit_attr(ld.ATTR_MAIL, email.value))
        for phone in data.phones:
            attr: str = ld.ATTR_MOBILE if ld.MOBILE_TYPE in phone.types else ld.ATTR_TELEPHONE
            lines.append(FormatEngineLdif.emit_attr(attr, phone.number))
        if data.organization:
            lines.append(FormatEngineLdif.emit_attr(ld.ATTR_O, data.organization))
        if data.department:
            lines.append(FormatEngineLdif.emit_attr(ld.ATTR_OU, data.department))
        if data.job_title:
            lines.append(FormatEngineLdif.emit_attr(ld.ATTR_TITLE, data.job_title))
        if data.note:
            lines.append(FormatEngineLdif.emit_attr(ld.ATTR_DESCRIPTION, data.note))
        lines.extend(self._address_lines(data))
        lines.extend(self._photo_lines(data))
        if data.uid:
            lines.append(FormatEngineLdif.emit_attr(ld.ATTR_UID, data.uid))
        return "\n".join(lines)

    @staticmethod
    def _photo_lines(data: CardContact) -> list[str]:
        """Emit inline photos as binary jpegPhoto attributes; external URIs have no directory attribute."""
        lines: list[str] = []
        for photo in data.photos:
            unwrapped: tuple[str, str] | None = DataUri.unwrap_base64(photo)
            if unwrapped is not None:
                lines.append(FormatEngineLdif.emit_attr_base64(ld.ATTR_JPEGPHOTO, unwrapped[1]))
        return lines
