...
class FolderShareEntrySchema(Schema):
    ...
    Rights can be expressed two ways, and at least one of them must be present in the request
    (an empty value is allowed and means "grant no rights", e.g. to revoke a user down to
    no access while keeping the share entry):
    ...
    def validate_permissions_or_rights(self, data: dict[str, Any], **kwargs: Any) -> None:  # pylint: disable=unused-argument
        """Require at least one of 'permissions' or 'rights' to be present.

        An empty value (``"permissions": []`` or ``"rights": {}``) is accepted: it explicitly
        grants no rights, which is different from omitting both keys entirely.
        """
        if "permissions" not in data and "rights" not in data:
