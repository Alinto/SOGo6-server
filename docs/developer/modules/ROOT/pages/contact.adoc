...
Beyond the JSON of the REST API (the `*Dict` serializers), a contact and an address book can be exchanged as vCard 3.0 (RFC 2426), vCard 4.0 (RFC 6350) or LDIF (RFC 2849). The codecs are hand-written (no external library) and live in `app/module/contact/format/`.
...
----
  format/
    ...
    vcard/  VcardFormatEngine.py  VcardConst.py
    ldif/   LdifFormatEngine.py   LdifConst.py
----
...
Writing chooses a version: `ContactSerializerVcard` is the abstract mapping, `ContactSerializerVcard3` / `ContactSerializerVcard4` carry the deltas (the VERSION value, the TYPE/PREF encoding, `KIND`, `ANNIVERSARY` vs `X-ANNIVERSARY`, the `GEO` form). `ContactsSerializerVcard` concatenates cards into a `.vcf`; `ContactListSerializerVcard3` / `...Vcard4` emit a list as a group card (`KIND:group` + `MEMBER` in 4.0, `X-ADDRESSBOOKSERVER-*` in 3.0).
...
`ContactSerializerLdif` / `ContactDeserializerLdif` map a contact onto `inetOrgPerson` attributes (cn, sn, givenName, mail, telephoneNumber / mobile, o, ou, title, description, the first postal address, uid); `ContactListSerializerLdif` / `ContactListDeserializerLdif` map a list onto `groupOfNames` (member DNs). A whole-document (address book) is handled by the `Contacts*Ldif` pair, which skips `groupOfNames` records; attributes outside the mapped subset are kept in `extra_properties`.

=== Fidelity and limits

Conversion is faithful for the fields the model carries, but it is *not* a byte-exact round-trip of the full RFCs. Not preserved: property parameters (LANGUAGE, ALTID, PID, MEDIATYPE, LABEL), property grouping (`item1.X-ABLabel`), embedded binary (a base64 photo / logo / sound / key - only URI values survive), partial / text dates, quoted-printable encoding, and on the LDIF side the change records (`changetype`) and every field outside the `inetOrgPerson` subset. Byte-exact fidelity - required for full Apple / Google / Outlook / Nextcloud interoperability - belongs to a later stage built on a stored raw vCard, alongside the CardDAV server.
