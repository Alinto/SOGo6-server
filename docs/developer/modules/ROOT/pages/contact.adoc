...
----
  ...
  ... all multipart/form-data, one "file" part, ?format=json|vcard3|vcard4|ldif|auto (default json)
----
...
`FileAdapterDatabase` (`app/utils/file/`) is the current backend; it stores blobs in `sogo_file_storage` (key, data, content_type, content_hash, timestamps) through the `DbFileStorage` manager (`app/manager/db/`), which is injected into its constructor (so the utils layer never imports a manager at runtime). The reference scheme is `sogo:file:<key>`; `matches` compares a stored `content_hash` (sha256) without loading the blob, which keeps the update round-trip cheap. A file backend (file share / WebDAV) can be substituted without touching the module - only the key-to-storage mapping differs.
...
*Lifecycle.* `save_all` itself never deletes - it would otherwise mutate the store before the owning row is committed, stranding the row on missing bytes if the commit then failed. Instead `update_contact` reclaims the blobs a write dropped (a replaced or removed photo) *after* the row commits (`_delete_dropped_files`), so the old `sogo_file_storage` row disappears immediately. A *deleted* contact is soft-deleted and keeps its blobs, like the tombstone itself; those, and any blob a post-commit delete missed, are reclaimed by `ModuleContact.clean()` (`FileAdapter.purge_orphans` drops every row no surviving contact references) - the same backstop sweep that purges tombstones and dangling distribution-list membership.
