...
* **app/__init__.py**: creates the Flask app, sets the swagger, sets global before_request and after_request.
...
* Manager does not import anything except utils/config. Manager must do its best to catch external
library exceptions and either raise SOGo exception or let the module/interface decide what to do
...
Admin API only has endpoints defined within `app/api/<version>/admin/` All the others folders are for the user API.
