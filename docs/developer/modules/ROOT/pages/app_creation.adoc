...
* Adds `process_config` to `g.process`
...
* `g.process`: Process configuration (database, Redis, etc.)
...
1. Adds process configuration to `g.process`
...
* `g.user`: Current user
* `g.process`: Process configuration
* `g.system_settings`: System-wide settings
* `g.default_domain`: Default domain settings
* `g.user_domain`: User's domain settings (currently same as default)
...
│  └─ Admin API → Add g.process → Continue
│
├─ [State: SOGO_OK]
│  └─ Load all configuration → g.process, g.system_settings, g.default_domain, g.user_domain
│
