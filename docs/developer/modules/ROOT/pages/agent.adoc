...
* *Agent* (`app/agent/Agent.py`) — the only file importing `celery`. Wraps the Celery calls (`send_task`, `revoke`, signal hooks) and exposes a domain API (`create_job`, `cancel`, `register_job_handler`, `register_all_job_handlers`, `register_lifecycle_hooks`, `start_worker`).
* *ClientAgent* (`app/manager/agent/ClientAgent.py`) — the single facade used by Flask. Composes Agent + persistency + canceller + cache; owns enqueue, the concurrency gate, `JobState` reads, and the large-blob store (`large_store`). The backend kind (a `JobLargeStorage`) is resolved from `SOGO_P_AGENT_LARGE_STORE` at wiring time and *injected* (like the Redis URL into `ClientRedis`); ClientAgent builds the instance, injecting the cache it holds into the in-memory backend.
...
The backend is *process-wide*. Its kind is read once from `SOGO_P_AGENT_LARGE_STORE` (`file` = filesystem, `in_memory` = Redis) at wiring time (`init_sogo` / the worker entrypoint), turned into a `JobLargeStorage` and injected into `ClientAgent` - the same pattern as the Redis URL injected into `ClientRedis` (no manager reads `process_config` itself). `ClientAgent` builds the instance because it holds the cache the in-memory backend needs *injected* (which also keeps that backend free of the `service -> ClientAgent` import cycle), and exposes it as `ClientAgent.large_store`. Callers reach it through the agent they already have: `sogo_agent().large_store` (worker, interface) or `self._agent.large_store` (a module with the agent injected). No per-job or per-ref backend dispatch - the reference only says *where* a blob is, not *how* to reach it.

The store is *bidirectional*:

* *Output* (large result: ICS, PDF...) — the worker `sogo_agent().large_store.save(content, content_type) -> JobLargeRef`, and stores `ref.to_dict()` under `large_result` plus light metadata. Fetched via `GET /jobs/<id>/result`.
...
// TODO: the FILE backend has no purge yet - a cleanup job is needed. IN_MEMORY expires via the Redis TTL.
...
The pattern: inject `MagicMock()` for `agent`, `persistency`, `canceller`, `cache`. For interface and worker tests, `patch(...)` `sogo_agent` and drive `sogo_agent().large_store.load` / `.save` directly (the store is the `ClientAgent.large_store` singleton).
