...
* *Input* (large upload: ICS import) — the module `save()` s the upload *before* enqueue (the worker no longer has the HTTP request) and puts the reference string in the request (`source_ref`). The worker `load()` s it, then `delete()` s it once consumed (an input blob is single-use). If enqueue fails, the module `delete()` s the ref so no blob dangles.
...
// TODO streaming: get_result loads the whole blob into memory. Serving via a streamed response would be better for large calendars under concurrent downloads. Deferred: payloads are size-bounded today.
