...
Resources don't have any limitations as they can be directly a resource name. ( /my_file@@@ext.pytz). But try your
best to keep it with alphanumerical only.

TIP: Underscore visibility in some software makes it a worse choice than hyphen
...
endpoint without the `/`, *Flask* will not return 404 NOT FOUND but a 302 FOUND
...
It will update the value of *PARAM1* to _value1_, and remove *PARAM2*
...
For *case 1* there is no problem. POST must give everything. PATCH that tries to remove a required parameter will fail
as the merged resource is not viable anymore.

For *case 2*, this is the same logic as *case 1*

For *case 3*, your implementation must check itself that there is at least one parameter at the end as
...
If a resource A has several resources B, any action on several resources B should be implemented
at resource B level.
...
[IMPORTANT]
====
Here are given the common status codes. If in your development you think about another status code
...
All responses are in JSON format and they all have the same basic structure, for all status codes
