...
External ICS subscriptions are stored as `CalCalendar` rows with `source_type=ics` and sync metadata in `sync_config` JSON (url, sync_interval_minutes, sync_status, last_sync). Events are mirrored into `sogo_events` by `SyncEngine` and served via `CalendarSourceIcsMirror` (read-only &#8212; API writes are rejected).
