...
`GET /calendars/{key}/export` serialises the calendar back into a VCALENDAR string (`text/calendar`). Recurring masters keep their `RRULE` intact — the expansion happens on the recipient side. Query parameters:
