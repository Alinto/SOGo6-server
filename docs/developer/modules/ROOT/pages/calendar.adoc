...
One rule is not implemented. RFC 5546 §2.1.5 orders successive replies from one attendee by `SEQUENCE`, then by `DTSTAMP` as a tie-break, and requires organizers to persist both per attendee. Since a REPLY never increments `SEQUENCE`, two replies to the same revision always tie, so `DTSTAMP` is the only discriminator - and nothing records it. Replies from one attendee are applied in arrival order. §2.1.5 names ordinary causes for that order to be wrong (queueing, retries), so this is reachable without anything replaying mail: a `tentative` then `accepted` pair delivered out of order settles on the wrong one.