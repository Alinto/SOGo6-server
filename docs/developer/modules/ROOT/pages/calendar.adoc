...
`GET /calendars/{key}/export` serialises the calendar back into a VCALENDAR string (`text/calendar`). The serialization is content-negotiated from the request `Accept` header (same shape as the contact export): an empty, `*/*` or `text/calendar` accept is served, any other explicit media type returns `406 Not Acceptable` (`ERROR_CALENDAR_EXPORT_FORMAT_UNSUPPORTED`). iCalendar being the only format, the negotiation is a single allow/deny — but the contract stays isomorphic with the contact module. Recurring masters keep their `RRULE` intact — the expansion happens on the recipient side. Query parameters:

* `start_date_time`, `end_date_time` — same semantics as the events list, restrict the export window.
* `download=true` — adds `Content-Disposition: attachment; filename="calendar-{key}.ics"` so browsers download the response instead of inlining it. Default is inline so the URL stays consumable by any HTTP client or `curl`.

The module fetches both events and tasks without expansion via `CalendarSource.get_events(..., expand=False)` and `get_tasks(..., expand=False)`, attaches them to the `CalCalendar` (its transient `events` field), then `CalCalendarSerializerIcal.serialize(calendar)` builds the VCALENDAR: it owns the calendar-level header (`PRODID`, `VERSION`, `CALSCALE`, the `X-WR-CALNAME`/`X-WR-CALDESC`/`X-WR-TIMEZONE` descriptors) and delegates the body — one `VEVENT`/`VTODO` per event — to `CalEventsSerializerIcal`. `expand=False` keeps recurring masters with their `RRULE` (no flat occurrences) and skips the Python date filter since the SQL fetch already bounds the range.

=== Import

`POST /calendars/{key}/import` accepts a `multipart/form-data` upload with a single `file` part. The view exposes `accepted_content_types = {"multipart/form-data"}` so the global `check_content_type` middleware skips its default JSON rule (the convention is documented in `CLAUDE.md`). The total request size is capped at the WSGI layer (`MAX_CONTENT_LENGTH`); the module enforces the per-import limit (`MAX_IMPORT_ICS_BYTES`), so the view only reads and decodes the part.

The pipeline goes `ApiCalendarImport` &#8594; `InterfaceApiCalendarCalendar.import_calendar` &#8594; `ModuleCalendar.import_calendar` &#8594; `SyncEngine.apply_ics(..., delete_missing=False, rewrite_owner_email=..., default_timezone=...)`. `apply_ics` deserialises the payload through `CalCalendarDeserializerIcal`, which reads the calendar-level header and delegates the body to `CalEventsDeserializerIcal`; only the parsed `events` are applied to the target calendar (the imported VCALENDAR's own name/timezone are not used — the import writes into an existing calendar). Key behaviours:

* `delete_missing=False`: the import is additive — events absent from the upload are kept (unlike the URL sync which mirrors).
* `rewrite_owner_email`: when `IMPORT_REWRITES_OWNERSHIP=True` (default, in `CalendarConst`), `apply_ics` works on the *deserialized models* — never on the raw iCal text — setting `event.organizer` to the importer and resetting `event.sequence` to 0 (RFC 5546 §2.1.4: SEQUENCE belongs to the organizer). The rewrite itself leaves attendees alone; their fate is decided by `remove_attendees` below.
* `remove_attendees`: when `IMPORT_REMOVES_ATTENDEES=True` (default, in `CalendarConst`), the ATTENDEE list of every imported event is cleared — events land as plain personal entries, so no iMIP can ever target the historical guests on a later edit. When `False`, attendees are preserved and the historical guest list is kept; iMIP would then only fire on subsequent explicit edits.
