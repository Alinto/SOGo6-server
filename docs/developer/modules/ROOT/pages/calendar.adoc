...
External ICS subscriptions are stored as `CalCalendar` rows with `source_type=ics` and sync metadata in `sync_config` JSON (url, sync_interval_minutes, sync_status, last_sync). Events are mirrored into `sogo_events` by `SyncEngine` via `CalendarSourceIcsMirror`. Write restrictions on ICS calendars are enforced by `CalendarAclEngine`, not by the source layer.

`POST /external-calendars/{key}/sync` triggers a synchronous sync. The sync engine fetches the ICS feed via `IcsFetcher` (SSRF-protected), diffs by (UID, recurrence_id), and applies inserts/updates/deletes. Sync status is tracked in `sync_config` and queryable via `GET /external-calendars/{key}/sync`.
...
The interface builds `CalendarUser` by resolving the owner from the calendar's `user_uid` via `ModuleUserProfile`.
