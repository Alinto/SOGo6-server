"""Unit tests for TaskCanceller — agent and persistency are mocked."""
...
def _state(status=TaskStatus.STARTED) -> TaskState:
    return TaskState(
...
def _canceller():
    ...
    return TaskCanceller(agent_mock, persistency_mock), agent_mock, persistency_mock
...
def test_cancel_does_nothing_when_already_terminal():
    canceller, agent_mock, persistency = _canceller()
    persistency.get.return_value = _state(status=TaskStatus.SUCCESS)
...
def test_cancel_sends_sigterm_then_returns_when_task_becomes_terminal():
    ...
    persistency.get.side_effect = [_state(status=TaskStatus.STARTED), _state(status=TaskStatus.CANCELED)]
...
def test_cancel_escalates_to_sigkill_when_grace_window_elapses():
    canceller, agent_mock, persistency = _canceller()
    persistency.get.return_value = _state(status=TaskStatus.STARTED)  # never terminal
...
def test_cancel_skips_sigkill_when_task_terminates_during_wait():
    ...
    persistency.get.side_effect = [
        _state(status=TaskStatus.STARTED),  # initial
        _state(status=TaskStatus.STARTED),  # poll 1
        _state(status=TaskStatus.CANCELED), # poll 2 — exit
    ]
