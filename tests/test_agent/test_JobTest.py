...
from tests.test_agent._helpers.JobTest import TaskTest
from app.agent.jobs.Job import AgentJobCancelled


def test_returns_payload_echo():
    out = TaskTest().process({"k": "v"}, user_uid="alice", job_id="t-1")
    assert out["echo"] == {"k": "v"}
    assert out["user_uid"] == "alice"
    assert out["job_id"] == "t-1"


def test_raises_when_payload_asks_for_failure():
    with pytest.raises(RuntimeError, match="forced failure"):
        TaskTest().process({"raise": True}, user_uid=None, job_id="t-2")


def test_sleeps_for_the_requested_duration():
    start = time.monotonic()
    TaskTest().process({"seconds": 0.05}, user_uid=None, job_id="t-3")
    assert time.monotonic() - start >= 0.05


def test_crash_dereferences_a_null_pointer():
    """A real crash would kill pytest; we mock ctypes to verify the intent."""
    with patch("tests.test_agent._helpers.JobTest.ctypes.string_at") as ctypes_call:
        TaskTest().process({"crash": True}, user_uid=None, job_id="t-4")
...
def test_cooperative_cancel_is_signalled_via_exception():
    ...
    AgentJobCancelled (raised by the SIGTERM handler installed at runtime). Here we
    simulate the propagation by patching ``time.sleep`` to raise."""
    with patch("tests.test_agent._helpers.JobTest.time.sleep", side_effect=AgentJobCancelled):
        with pytest.raises(AgentJobCancelled):
            TaskTest().process({"seconds": 60}, user_uid=None, job_id="t-5")
...
def test_soft_timeout_propagates_to_the_caller():
    ...
    class _Soft(Exception):
        pass
    with patch("tests.test_agent._helpers.JobTest.time.sleep", side_effect=_Soft):
        with pytest.raises(_Soft):
            TaskTest().process({"seconds": 60}, user_uid=None, job_id="t-6")
