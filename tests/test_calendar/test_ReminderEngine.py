...
def _reminder(event_key="evt-key", method=ReminderMethod.POPUP, minutes_before=15,
              trigger_at=None, date_start=_UNSET, date_end=_UNSET):
...
def test_lookahead_extends_active_window():
    event = _make_event(date_end=_dt(2026, 6, 1, 11, 0))
...
def test_no_date_end_reminder_stays_active():
    event = _make_event(date_end=None)
    now = _dt(2026, 6, 1, 23, 0)
    results = ReminderEngine().compute_active(
        reminders=[_reminder(trigger_at=_dt(2026, 6, 1, 9, 45), date_end=None)],
        events_by_key={"evt-key": event},
        now=now,
    )
    assert len(results) == 1
