...
def test_daily_basic(engine):
    ...
    starts = [e.start_date for e in result]
    assert len(result) == 5
...
def test_daily_until(engine):
    ...
    assert all(e.start_date <= until for e in result)
...
def test_monthly_same_day(engine):
    ...
    assert all(e.start_date.day == 15 for e in result)
    assert len(result) == 4
...
def test_monthly_bymonthday(engine):
    ...
    starts = [e.start_date for e in result]
    assert all(e.start_date.day == 10 for e in result)
...
def test_monthly_byday_positional(engine):
    ...
    result = engine.expand(master, _dt(2026, 1, 1), _dt(2026, 3, 31))
    starts = [e.start_date for e in result]
    assert all(e.start_date.weekday() == 0 for e in result)  # all Mondays
    assert _dt(2026, 1, 5) in starts
...
def test_recurrence_id_override(engine):
    ...
    override = CalEvent(
        ...
        start_date=_dt(2026, 1, 6, 14),
        end_date=_dt(2026, 1, 6, 15),
        recurrence_id=_dt(2026, 1, 6),
    )
    result = engine.expand(master, _dt(2026, 1, 5), _dt(2026, 1, 7), overrides=[override])
    jan6 = [e for e in result if e.start_date.date() == _dt(2026, 1, 6).date()]
    assert len(jan6) == 1
    assert jan6[0].title == "Modified Jan 6"
    assert jan6[0].start_date.hour == 14
...
def test_occurrence_copy_fields(engine):
    ...
    result = engine.expand(master, _dt(2026, 1, 1), _dt(2026, 1, 31))
    for occ in result:
        assert occ.recurrence_id == occ.start_date
...
def test_occurrence_inherits_master_fields(engine):
    master = CalEvent(
        ...
        start_date=_dt(2026, 1, 5),
        end_date=_dt(2026, 1, 5, 10),
...
def test_secondly(engine):
    ...
    master = CalEvent(
        uid="s@test", title="T",
        start_date=dtstart,
        end_date=datetime(2026, 1, 5, 9, 0, 10, tzinfo=_UTC),
    ...
    assert len(result) == 3
    assert result[0].start_date == datetime(2026, 1, 5, 9, 0, 0, tzinfo=_UTC)
    assert result[1].start_date == datetime(2026, 1, 5, 9, 0, 30, tzinfo=_UTC)
    assert result[2].start_date == datetime(2026, 1, 5, 9, 1, 0, tzinfo=_UTC)
...
def test_hourly(engine):
    ...
    assert len(result) == 4
    assert result[0].start_date == _dt(2026, 1, 5, 9)
    assert result[1].start_date == _dt(2026, 1, 5, 10)
    assert result[2].start_date == _dt(2026, 1, 5, 11)
    assert result[3].start_date == _dt(2026, 1, 5, 12)
...
def test_byyearday_negative(engine):
    ...
    result = engine.expand(master, _dt(2026, 1, 1), _dt(2027, 12, 31))
    assert all(e.start_date.month == 12 and e.start_date.day == 31 for e in result)
    assert len(result) == 2
...
def test_hourly_byminute_filters_minutes(engine):
    ...
    result = engine.expand(master, datetime(2026, 1, 1, tzinfo=_UTC), datetime(2026, 12, 31, tzinfo=_UTC))
    assert len(result) == 3
    assert all(e.start_date.minute == 0 for e in result)
    assert result[1].start_date == datetime(2026, 1, 5, 10, 0, 0, tzinfo=_UTC)
...
def test_daily_byhour_bysetpos_selects_last_per_day(engine):
    ...
    result = engine.expand(master, datetime(2026, 1, 1, tzinfo=_UTC), datetime(2026, 12, 31, tzinfo=_UTC))
    assert len(result) == 3
    assert all(e.start_date.hour == 17 for e in result)
...
def test_get_min_date_returns_start_date(engine):
    master = _master(_dt(2026, 3, 10), _dt(2026, 3, 10, 10))
    assert engine.get_min_date(master) == master.start_date


def test_get_max_date_no_rrule(engine):
    master = _master(_dt(2026, 3, 10), _dt(2026, 3, 10, 10))
    assert engine.get_max_date(master) == master.end_date
